...
static struct trans_table_t * get_trans_table(addr_t index, struct page_table_t *page_table) {
	int i;
	for (i = 0; i < page_table->size; i++) { // Iterate every row in the provided page table
		if(index == page_table->table[i].v_index) { // Check if the virtual index of the current row matches the provided segment index
...
static int translate(
		...
		struct pcb_t * proc) {  // Process that uses the given virtual address
...
addr_t alloc_mem(uint32_t size, struct pcb_t *proc) {
	...
	uint32_t num_next_lv = ((size % PAGE_SIZE) == 0) ? size / PAGE_SIZE :
		size / PAGE_SIZE + 1; // Number of next level page tables we will use
	...
	*/
	...
	for(i = 0; i < NUM_PAGES; i++) { // Iterate through every page in _mem_stat array
		if(_mem_stat[i].proc == 0) { // If the current page is free
			num_avail_next_lv++; // Increase count of free pages
			if(num_avail_next_lv == num_next_lv && proc->bp + num_next_lv * PAGE_SIZE <= RAM_SIZE) { // If we have enough free pages and enough space in virtual memory
	...
	if (mem_avail) { // If we can allocate new memory
		...
		*/
		...
		for(i = 0; i < NUM_PAGES; i++) { // Iterate through every page in _mem_stat array
			if(_mem_stat[i].proc == 0) { // If the current page is free
				...
				int j;
				for(j = 0; j < page_table->size; j++) { // Iterate through every row in the segment table
					if (page_table->table[j].v_index == page_idx) { // If a matching entry is found in the page table
				...
				if (!found) { // If no matching entry is found in the page table
				...
				if (num_alloc_next_lv == num_next_lv) { // If we have allocated enough memory pages
